...
export class GHCRProvider extends BaseProvider {
  ...
  private async getPackageVersions(packageName: string): Promise<Array<{ id: number; tags: string[]; created_at: string }>> {
    ...
    try {
      response = await this.httpClient.get<Array<{
        id: number;
        name: string;
      ...
      if (error instanceof Error && (error.message.includes('404') || error.message.includes('Not Found'))) {
        ...
        try {
          ...
          const orgStatusCode = orgError instanceof Error && 'statusCode' in orgError ? (orgError as any).statusCode : 'unknown';
