...
class CleanupEngine {
     ...
     */
    async hasExcludedTagsForManifest(packageName, digest) {
        ...
        const imagesWithSameManifest = this.allDiscoveredImages.filter((img) => img.manifest.digest === digest && img.package.name === packageName);
     ...
     */
    async canDeleteManifest(packageName, digest) {
        if (!this.config.excludeTags || this.config.excludeTags.length === 0) {
